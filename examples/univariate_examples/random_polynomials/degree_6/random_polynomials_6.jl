...
F = hcat([c6*x^6+c5*x^5+c4*x^4+c3*x^3+c2*x^2+c1*x+c0])
bp = [CCi(1/2), CCi(9/7), CCi(9/7), CCi(45/7), CCi(45/56), CCi(-2/3), CCi(1/3)]
x = [CCi(.0720838,-.481488)]
v1 = vertex(bp,[x])
n_nodes = 6;

@gap("Read(\"~/Documents/GitHub/certified_monodromy_comp/examples/univariate_examples/random_polynomials/degree_6/random_polynomials_6.txt\");")
@gap("G;")
@gap("StructureDescription(G);") # S6
@gap("GaloisWidth(G);") # 6


path = safe_path("~/Documents/GitHub/certified_monodromy_comp/examples/univariate_examples/random_polynomials/degree_6/results_$(n_nodes)_nodes.txt")
using GAP
...
open(path, "w") do file
    ...
    for i in 1:100
        try
            v1 = vertex(bp,[x])
            vs = parameter_points(v1, 7, n_nodes)
            ...
            str_convert(perms, "~/Documents/GitHub/certified_monodromy_comp/examples/univariate_examples/random_polynomials/degree_6/random_polynomials_6_$(n_nodes)nodes", "H")

            filename = expanduser("~/Documents/GitHub/certified_monodromy_comp/examples/univariate_examples/random_polynomials/degree_6/random_polynomials_6_$(n_nodes)nodes.txt")
