...
F = hcat([c1*x^8+c2*x^7+c3*x^6+c4*x^5+c5*x^4+c4*x^3+c3*x^2+c2*x+c1])
bp = [CCi(1/2), CCi(9/7), CCi(9/7), CCi(3/2), CCi(3/17)]
x = [CCi(.750923,-.660389)]
v1 = vertex(bp,[x])
    ...
    open(path, "w") do file
        ...
        while i <= 100
            try
                ...
                edges = track_complete_graph(F, r, vs,8; predictor=false)
